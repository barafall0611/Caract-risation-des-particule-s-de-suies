# -*- coding: utf-8 -*-
"""
Created on Wed Dec 17 11:15:40 2025
...
img_path = r"C:\Users\bara.fall\Desktop\traiter 04-12-2025\225770\225770-8800X-0018.jpg"
...
_, vesicles = cv2.threshold(img_eq, 240, 255, cv2.THRESH_BINARY)
...
def mouse_callback(event, x, y, flags, param):
    ...
    global drawing_poly, poly_points
    ...
    if flags & cv2.EVENT_FLAG_CTRLKEY:
        ...
        # --- Fin tracé → création de l’agrégat
        if event == cv2.EVENT_LBUTTONUP and drawing_poly:
            ...
            mask_editable = cv2.bitwise_and(
                mask_editable, cv2.bitwise_not(poly_mask)
            )
            ...
            cnts, _ = cv2.findContours(
                mask_editable, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE
            )
    ...
    # ====================================================
    # 🟥 CTRL + RECTANGLE = SUPPRESSION ZONALE
    # ====================================================
    ...
    if event == cv2.EVENT_MOUSEMOVE and drawing_rect:
        temp = img_display.copy()
        cv2.rectangle(temp, rect_start, (x, y), (0, 0, 255), 1)
        cv2.imshow("Édition interactive", temp)
        return

    if event == cv2.EVENT_LBUTTONUP and drawing_rect:
        drawing_rect = False
        x1, y1 = rect_start
        x2, y2 = x, y
        ...
        zone = np.zeros_like(mask_editable)
        zone[y_min:y_max, x_min:x_max] = 255
        mask_editable = cv2.bitwise_and(mask_editable, cv2.bitwise_not(zone))
        ...
        cv2.imshow("Édition interactive", img_display)
        print("🧹 Suppression zonale")
        return

    # ====================================================
    # 🔹 CLIC GAUCHE SIMPLE = SUPPRESSION / RÉACTIVATION
    # ====================================================
    if event == cv2.EVENT_LBUTTONDOWN:
        label_img = measure.label(mask_editable)
        lbl = label_img[y, x]
        if lbl == 0:
            return

        region = (label_img == lbl).astype(np.uint8) * 255

        if region[y, x] > 0:
            mask_editable = cv2.bitwise_and(mask_editable, cv2.bitwise_not(region))
            color = (0, 0, 255)
            print("❌ Agrégat supprimé")
        else:
            mask_editable = cv2.bitwise_or(mask_editable, region)
            color = (0, 255, 0)
            print("✅ Agrégat réactivé")

        img_display[:] = img_initial_annot.copy()
        cnts, _ = cv2.findContours(mask_editable, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(img_display, cnts, -1, color, 1)
        cv2.imshow("Édition interactive", img_display)
        return

    # ====================================================
    # 🔹 CLIC DROIT = AJOUT AUTOMATIQUE (OPTIONNEL)
    # ====================================================
    if event == cv2.EVENT_RBUTTONDOWN:
        ...
        cnts, _ = cv2.findContours(mask_editable, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        cv2.drawContours(img_display, cnts, -1, (0, 255, 0), 1)
        cv2.drawContours(img_display, [cnt + np.array([[offset]])], -1, (255, 0, 255), 2)

        cv2.imshow("Édition interactive", img_display)
        print("➕ Agrégat ajouté automatiquement")
        return
